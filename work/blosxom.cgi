...
$blog_title = "Ocean of Awareness";

# What's this blog's description (for outgoing RSS feed)?
$blog_description = "Ocean of Awareness.";
...
} ## end while ( $path_info[0] and $path_info[0] =~ /^[a-zA-Z].*$/...)
...
$template = sub {
    ...
    return join '',
        ( $template{$flavour}{$chunk} || $template{error}{$chunk} || '' );
};
...
if ( $plugin_dir and opendir PLUGINS, $plugin_dir ) {
    ...
    } ## end foreach my $plugin ( grep { /^\w+$/ && -f "$plugin_dir/$_"...})
    closedir PLUGINS;
} ## end if ( $plugin_dir and opendir PLUGINS, $plugin_dir )
		...
		and ( -r $File::Find::name )
                )
            {
                ...
                (   $show_future_entries
                    ...
                    or stat( "$static_dir/$1/index." . $static_flavours[0] )
                    ...
                    and $static_entries
                    and
                    $indexes{ ( $1 ? "$1/" : '' ) . "$2.$file_extension" } =
                    1

            } ## end if ( $File::Find::name =~ ...)
            else {
                !-d $File::Find::name
                    and -r $File::Find::name
                    and $others{$File::Find::name} =
                    stat($File::Find::name)->mtime;
    ...
    unless ( defined($skip) and $skip ) {
        ...
        if ( $currentdir =~ /(.*?)([^\/]+)\.(.+)$/ and $2 ne 'index' ) {
            ...
            $files{"$datadir/$1$2.$file_extension"}
                and %f =
                ( "$datadir/$1$2.$file_extension" =>
                    $files{"$datadir/$1$2.$file_extension"} );
        } ## end if ( $currentdir =~ /(.*?)([^\/]+)\.(.+)$/ and $2 ne...)
        else {
            $currentdir =~ s!/index\..+$!!;
        }
        ...
        foreach my $path_file ( &$sort( \%f, \%others ) ) {
            ...
            if ( -f "$path_file" && $fh->open("< $path_file") ) {
                ...
                $fh->close;
                $raw = "$title\n$body";
            } ## end if ( -f "$path_file" && $fh->open("< $path_file") )
            ...
            $ne--;
        } ## end foreach my $path_file ( &$sort( \%f, \%others ) )
