...
<html>
  ...
  <body style="max-width:850px">
    ...
    which we hope to solve the rest of it.
    </p>
    <pre><tt>
    ...
    The <tt>read()</tt> method returns the current position
    in the input.
    ...
    <p>Marpa claim to be "left-eidetic",
    ...
    But for our purpose here, this is a more convenient interface.
    </p>
    <p>We want to determine how many <tt>c</tt> characters we are looking for,
    which depends on the number of 
    <tt>a</tt> and <tt>b</tt> characters that we have already seen.
    What we really want to know is how long the most recent
    <tt>&lt;ABs&gt;</tt> symbol was.
    </p>
    <p>Marpa has a <tt>last_completed_span()</tt> method is just what
    we need.
    ...
    Our input is a string referred by <tt>$input</tt>.
    ...
    <p>
    Marpa requires that
    every symbol must be tied in some way to the input.
    This is so that it can report errors.
    The tie-in can be hack-ish or completely artificial,
    if necessary.
    In this application, our symbol instance is tied into
    the input in a very natural way --
    we tell Marpa that the symbol is at <tt>$pos</tt> in the input,
    ...
    loop.
    (There will only be one in case of an error.)
    Once done, the <tt>EVENT</tt> loop will hand control
    over to the outer loop which will call the <tt>resume</tt>
    method.
    </p>
    <h2>TODO</h2>
    <p>TODO<footnote>
