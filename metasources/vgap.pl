...
with a new protocol stack.
    ...
    <li>The inter-part must be either a sequence of inter-comments
    and staircases.
    </li>
    ...
    and unambiguity is a good way of avoiding them.<footnote>
    This, by the way,
    ...
    <p>
    Keeping the parser unambiguous means that the grammar
    ...
    For example,
    our Ruby Slippers "exception mechanism" is set up so
    ...
    an ambiguity between a structural and a meta-comment is entirely
    possible.
    </footnote>
    </p>
    <p>
    Similarly, the Ruby Slippers are used to quarantee that
    any set of tokens containing either
    a <tt>&lt;BadComment&gt;</tt>
    or a
    <tt>&lt;BlankLine&gt;</tt> is a singleton.
    In this way, the lexer guarantees that,
    while tokenization can be ambiguous,
    the parse never is.
    </p>
    <h2>Code</h2>
